...
contract WifiAllocation {
    ...
    uint constant priceScalingFactor = 100;
    uint constant bwScalingFactor = 100;
    uint public minPrice;
    uint public currentPrice = minPrice * priceScalingFactor;
    ...
    constructor(uint _totalBandwidth, uint _minPrice, uint _burstPrice) public {
    ...
    //     // Param1: current unit price of bandwidth (scaled by priceScalingFactor)
    ...
    //     return (currentPrice, totalDesiredBandwidth, totalDesiredDynamic, totalDesiredNonDynamic);
    // }
    ...
    function updateBalance() public {
        for(uint i = 0; i < numUsers; i++) {
            if(isActive[i]) {
                ...
                uint currentFee = currentPrice * users[i + 1].receivedBandwidth * (currentTime - users[i + 1].lastPaymentTime) / (priceScalingFactor * bwScalingFactor);
                ...
                currentFee += burstPrice * users[i + 1].burst * (currentTime - users[i + 1].lastPaymentTime);
    ...
    function connectionInitialisation(uint bandwidth, uint thresholdBid, uint burst) internal requirePayment returns (uint userID) {
        ...
        // In this case, the thresholdBid is compared with desiredBandwidth * currentPrice to determine whether the user is dynamic
        if(currentPrice * bandwidth / priceScalingFactor > thresholdBid) {
            isDynamic.push(true);
        ...
        burstVol.push(burst);
        updatePrice(bandwidth * bwScalingFactor, true);
        performAllocation();
        updateBalance();
    }
    ...
    function connectionRenewal(uint userID, uint bandwidth, uint thresholdBid, uint burst) internal {
        ...
        // ***NOTE***: receivedBandwidth is scaled by bwScalingFactor
        bool isPositive = (bandwidth * bwScalingFactor >= users[userID].receivedBandwidth);
        updatePrice(isPositive ? (bandwidth * bwScalingFactor - users[userID].receivedBandwidth) : (users[userID].receivedBandwidth - bandwidth * bwScalingFactor), isPositive);
        // --------------------------------------
        ...
        // updatePrice(isPositive ? (bandwidth - users[userID].desiredBandwidth) * bwScalingFactor : (users[userID].desiredBandwidth - bandwidth) * bwScalingFactor, isPositive);
        performAllocation();
        updateBalance();
    }
    ...
    function connectionTermination(uint userID) internal {
        ...
        // uint deltaBandwidth = users[userID].desiredBandwidth * bwScalingFactor;
    ...
    function demandNotExceedSupply() internal {
        for(uint i = 0; i < numUsers; i++) {
            if(isActive[i]) {
                actualPrices[i] = currentPrice;
                allocatedBandwidth[i] = users[i + 1].desiredBandwidth * bwScalingFactor;
                users[i + 1].receivedBandwidth = users[i + 1].desiredBandwidth * bwScalingFactor;
            }
        }
    }
    ...
    function demandExceedSupply() internal {
        ...
        if(totalDesiredNonDynamic >= totalBandwidth) {
            // Ignore isDynamic[] and actualPrices[], and all users are charged currentPrice
            // Proportionally allocate the bandwidth according to desired bandwidth
            for(uint i = 0; i < numUsers; i++) {
                if(isActive[i]) {
                    actualPrices[i] = currentPrice;
                    allocatedBandwidth[i] = users[i + 1].desiredBandwidth * totalBandwidth * bwScalingFactor / totalDesiredBandwidth;
            ...
            // weight = totalDesiredDynamic * priceScalingFactor / (totalBandwidth - totalDesiredNonDynamic);
            // weightedPrice = weight * currentPrice / priceScalingFactor;
            weightedPrice = totalDesiredDynamic * currentPrice / (totalBandwidth - totalDesiredNonDynamic);
            for(uint i = 0; i < numUsers; i++) {
                if(isActive[i]) {
                    if(isDynamic[i]) {
                        actualPrices[i] = currentPrice;
                        allocatedBandwidth[i] = users[i + 1].desiredBandwidth * (totalBandwidth - totalDesiredNonDynamic) * bwScalingFactor / totalDesiredDynamic;
                        users[i + 1].receivedBandwidth = allocatedBandwidth[i];
                    } else {
                        // actualPrices[i] = weight * currentPrice / priceScalingFactor;
    ...
    function updatePrice(uint deltaBandwidth, bool isPositive) internal {
        // uint prevPrice = currentPrice;
        if(totalDesiredBandwidth <= totalBandwidth) {
            currentPrice = minPrice * priceScalingFactor;
        } else {
            if(isPositive) {
                ...
                if(totalDesiredBandwidth * bwScalingFactor - deltaBandwidth < totalBandwidth * bwScalingFactor) {
                    currentPrice = currentPrice * totalDesiredBandwidth / totalBandwidth;
                } else {
                    currentPrice = currentPrice * (totalBandwidth * bwScalingFactor + deltaBandwidth) / (totalBandwidth * bwScalingFactor);
                    // currentPrice = currentPrice * totalDesiredBandwidth * bwScalingFactor / (totalDesiredBandwidth * bwScalingFactor - deltaBandwidth);
                }
            } else {
    ...
    function updateIsDynamic() internal {
        ...
        if(totalDesiredNonDynamic >= totalBandwidth) {
            for(uint i = 0; i < numUsers; i++) {
                if(isActive[i]) {
                    // In this case, the thresholdBid is compared with desiredBandwidth * currentPrice to determine whether the user is dynamic
                    if(!isDynamic[i] && currentPrice * users[i + 1].desiredBandwidth / priceScalingFactor > users[i + 1].thresholdBid) {
                        isDynamic[i] = true;
                        totalDesiredNonDynamic -= users[i + 1].desiredBandwidth;
                        totalDesiredDynamic += users[i + 1].desiredBandwidth;
                    } else if (isDynamic[i] && currentPrice * users[i + 1].desiredBandwidth / priceScalingFactor <= users[i + 1].thresholdBid) {
                        isDynamic[i] = false;
                        totalDesiredDynamic -= users[i + 1].desiredBandwidth;
                        totalDesiredNonDynamic += users[i + 1].desiredBandwidth;
                    }
            ...
            if(totalDesiredNonDynamic < totalBandwidth) {
                ...
                for(uint i = 0; i < numUsers; i++) {
                    if(isActive[i]) {
                        ...
                        if(!isDynamic[i] && weightedPrice * users[i + 1].desiredBandwidth / priceScalingFactor > users[i + 1].thresholdBid) {
                            isDynamic[i] = true;
                            totalDesiredNonDynamic -= users[i + 1].desiredBandwidth;
                            totalDesiredDynamic += users[i + 1].desiredBandwidth;
                            hasChanged = true;
                        } else if(isDynamic[i] && weightedPrice * users[i + 1].desiredBandwidth / priceScalingFactor <= users[i + 1].thresholdBid) {
                            isDynamic[i] = false;
                            totalDesiredDynamic -= users[i + 1].desiredBandwidth;
                            totalDesiredNonDynamic += users[i + 1].desiredBandwidth;
                            hasChanged = true;
